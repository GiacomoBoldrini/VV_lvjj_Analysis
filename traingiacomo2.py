...
if args.crossval :
    output_dir = "/home/giacomo/tesi1/DNN_test/Data2"
    class_weights = class_weight.compute_class_weight('balanced',np.unique(y),y)
    ...
    for models in args.model:
        ...
        for train,test in kf.split(x):
            ...
            x_test = x[test]
            y_test = y[test]
            model = MT.ModelTemplates(models, 11)
...
DNN_Plotter.Loss_val_loss(args.model, "loss", "loss model", output_dir1 , args.crossval)
plt.clf()
plt.close()
DNN_Plotter.Loss_val_loss(args.model, "val_loss", "val_loss model", output_dir1 , args.crossval)
plt.clf()
plt.close()
